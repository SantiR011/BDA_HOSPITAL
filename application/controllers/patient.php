...

{
	...
	function view_prescripcion($param1 = '', $param2 = '', $param3 = '')

	{
		...
		$page_data['page_name']     = 'view_prescripcion';

		$page_data['page_title']    = ('Ver prescripción');

		$page_data['prescripcions'] = $this->db->get_where('prescripcion', array(
...

	{
		...
		$page_data['page_title'] = ('Ver Doctor');
...

	{
		...
		if ($param1 == 'paypal_ipn') {
			...
			if ($this->paypal->validate_ipn() == true) {
				...
				$data2['paciente_id']     = $this->crud_model->get_type_name_by_id('invoice', $invoice_id, 'paciente_id');
...

	{
		...
		if ($param1 == 'update_profile_info') {
			...
			$this->db->where('paciente_id', $this->session->userdata('paciente_id'));

			$this->db->update('paciente', $data);

			$this->session->set_flashdata('flash_message', ('Profile Updated'));

			redirect(base_url() . 'index.php?paciente/manage_profile/', 'refresh');
		...
		if ($param1 == 'change_password') {
			...
			$current_password = $this->db->get_where('paciente', array(
